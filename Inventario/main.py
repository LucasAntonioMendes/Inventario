...
#abrindo
app_img = Image.open('./Imagem/inventorio.png')
app_img = app_img.resize((45,45))
app_img = ImageTk.PhotoImage(app_img)

app_logo = Label(frameCima, image = app_img,text ="Iventario Doméstico", width = 900, compound = LEFT, relief = RAISED, anchor = NW, font=('Verdana 20 bold'),bg=co1,fg = co4)
...
def atulizar():
    try:
        ...
        def update():
            ...
            messagebox.showinfo('Sucesso,''Os dados foram atulizados com sucesso')
            e_nome.delete(0, 'end')
            e_local.delete(0, 'end')
            e_descricao.delete(0, 'end')    
            e_model.delete(0, 'end')
        ...
        botao_confirmar = Button(frameMeio, command=update ,text="Confirmar".upper(), compound = LEFT, anchor= NW,width = 13,height = 1, overrelief = RIDGE, font=('Ivy 8'),bg = co2, fg = co1)
...
def deletar():
    try:
        treev_dados = tree.focus()
        treev_discionario = tree.item(treev_dados)
        treev_lista = treev_discionario['values']
        valor = treev_lista[0] 
...
img_add = Image.open('./Imagem/add.png')
...
img_update = Image.open('./Imagem/update.png')
img_update = img_update.resize((20,20))
img_update = ImageTk.PhotoImage(img_update)
botao_atulizar = Button(frameMeio,command = atulizar ,image = img_update, compound = LEFT, anchor= NW, text="Atulizar".upper(),width = 95, overrelief = RIDGE, font=('Ivy 8'),bg = co1, fg = co0)
botao_atulizar.place(x=330 , y = 50)

#Botão deletar

img_delete = Image.open('./Imagem/delete.png')
img_delete = img_delete.resize((20,20))
img_delete = ImageTk.PhotoImage(img_delete)
botao_deletar = Button(frameMeio, image = img_delete,command = deletar, compound = LEFT, anchor= NW, text="Deletar".upper(),width = 95, overrelief = RIDGE, font=('Ivy 8'),bg = co1, fg = co0)
botao_deletar.place(x=330 , y = 90)

#Botão ver item

img_item = Image.open('./Imagem/item.png')
img_item = img_item.resize((20,20))
img_item = ImageTk.PhotoImage(img_item)
botao_ver = Button(frameMeio, image = img_item,command = ver_imagem, compound = LEFT, anchor= NW, text="Ver item".upper(),width = 95, overrelief = RIDGE, font=('Ivy 8'),bg = co1, fg = co0)
botao_ver.place(x=330 , y = 221)

#label Quantidade total e valores
l_total = Label(frameMeio, width = 10 , height = 2, anchor= CENTER, font= ('Ivy 25 bold'), bg = co7, fg = co1, relief = FLAT)
l_total.place(x = 450, y = 13)
